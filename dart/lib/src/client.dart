...
class AcronymgeneratorValidationException implements Exception {
  final List<String> errors;

  AcronymgeneratorValidationException(this.errors);

  @override
  String toString() => 'AcronymgeneratorValidationException: ${errors.join("; ")}';
}
...
/// Acronym Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/acronymgenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [text] (required) - The text or phrase to generate acronyms from (max 200 characters) [maxLength: 200]
class AcronymgeneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'text': ValidationRule(type: 'string', required: true, maxLength: 200),
  };

  AcronymgeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/acronymgenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AcronymgeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AcronymgeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Acronym Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<AcronymgeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AcronymgeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AcronymgeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AcronymgeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AcronymgeneratorException('Resource not found');
      } else {
        throw AcronymgeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AcronymgeneratorException) rethrow;
      throw AcronymgeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Acronym Generator API client.
class AcronymgeneratorException implements Exception {
  final String message;

  AcronymgeneratorException(this.message);

  @override
  String toString() => 'AcronymgeneratorException: $message';
}
